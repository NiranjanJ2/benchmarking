...
docker run -d --name "$CONTAINER_NAME" -e "NODE_ENV=ctf" -e "CTF_KEY=$CTF_KEY" -p "${PORT}:3000" bkimminich/juice-shop
attempts=0
until curl -s -o /dev/null -w "%{http_code}" "http://localhost:$PORT" 2>/dev/null | grep -qE "^(200|301|302)$"; do
  sleep 3; attempts=$((attempts+1)); [[ $attempts -gt 20 ]] && { echo "ERROR: Juice Shop did not start"; exit 1; }
done
...
echo "[$(date '+%H:%M:%S')] Baseline saved — $total challenges | Target: http://host.docker.internal:$PORT | Score with: ./juice_shop_score.sh"
